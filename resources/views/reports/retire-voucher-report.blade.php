...
<head>
    ...
    <title>Retirement Voucher</title>
    <style>
        * {
    ...
    <div style="width: 100%; padding-top: 10px; padding-bottom: 10px">
        ...
        <div class="width-90-per" style="margin: auto">
            ...
            <table class="tableBordered iTableBordered width-100-per" style="margin-top: 10px">
                ...
                <tr>
                    <td colspan="3">VOI</td>
                    <td colspan="8">{{$data->retire_voucher->id}}</td>
                </tr>
                <tr>
...
{{--                    <td class="border-none"></td>--}}
        ...
        <table style="width: 100%; margin-top: 40px;" class="table-bordered">
            ...
            @foreach($data->retire_voucher->retire_liabilities as $retire_liability)
                <tr style="font-weight: bold" class="text_font_family">
                    <td class="table-bordered bind-data"
                        style="text-align: center;vertical-align: baseline;padding: 10px">{{\Illuminate\Support\Carbon::parse($retire_liability->liability_value_date)->format('d/m/Y')}}</td>
                    <td class="table-bordered bind-data"
                        style="text-align: left;vertical-align: baseline; padding: 10px">{{isset($retire_liability->company_id) ? $retire_liability->company->name : $retire_liability->employee->first_name .' '.$retire_liability->employee->last_name}}</td>
                    <td class="table-bordered bind-data"
                        style="text-align: left;vertical-align: baseline; padding: 10px">
                        {{$retire_liability->details}}
                    </td>
                    <td class="table-bordered bind-data"
                    ...
                    <td class="table-bordered bind-data"
                        style="text-align: center;vertical-align: baseline; padding: 10px">
                        {{preg_replace("/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/i", "$1,", explode('.', $retire_liability->amount)[0])}}
                    </td>
                    <td class="table-bordered bind-data"
                        style="text-align: center;vertical-align: baseline; padding: 10px">{{((explode('.', $retire_liability->amount)[1] ?? '00') * 10) === 0 ? '00' : (explode('.', $retire_liability->amount)[1] ?? '00') * 10}}
                    </td>
                </tr>
            @endforeach
            <tr>
