    ...
    public function summaryNonPersonalAdvances($params)
    {
        ...
        if (isset($params['inputs']['admin_segment_ids'])) {
            ...
            $pv = PaymentVoucher::whereIn('admin_segment_id', json_decode($params['inputs']['admin_segment_ids']))
                ...
                ->join('treasury_payee_vouchers', 'treasury_receipt_voucher_id_vouchers.id', '=', 'treasury_payee_vouchers.receipt_voucher_id_voucher_id')
                ->join('admin_segments', 'admin_segments.id', '=', 'treasury_receipt_voucher_id_vouchers.admin_segment_id')
                ->whereNull('company_id')
                ->where('type', AppConstant::VOUCHER_TYPE_NON_PERSONAL_VOUCHER)
                ->groupby('treasury_receipt_voucher_id_vouchers.admin_segment_id');
            ...
            $pv = PayeeVoucher::where('admin_segment_id', $params['inputs']['admin_segment_id'])
                ...
                ->join('treasury_receipt_voucher_id_vouchers', 'treasury_receipt_voucher_id_vouchers.id', '=', 'treasury_payee_vouchers.receipt_voucher_id_voucher_id')
                ->join('hr_employees', 'hr_employees.id', '=', 'treasury_payee_vouchers.employee_id')
                ->join('admin_segments', 'admin_segments.id', '=', 'treasury_receipt_voucher_id_vouchers.admin_segment_id')
                ->where('type', AppConstant::VOUCHER_TYPE_NON_PERSONAL_VOUCHER)
                ->groupby(['admin_segment_id', 'hr_employees.id']);
        }else {
            $pv = PaymentVoucher::join('treasury_payee_vouchers', 'treasury_receipt_voucher_id_vouchers.id', '=', 'treasury_payee_vouchers.receipt_voucher_id_voucher_id')
                ->join('admin_segments', 'admin_segments.id', '=', 'treasury_receipt_voucher_id_vouchers.admin_segment_id')
                ->whereNull('company_id')
                ->where('type', AppConstant::VOUCHER_TYPE_NON_PERSONAL_VOUCHER)
...
dd($pv);
    ...
    public function summaryPersonalAdvances($params)
    {
        ...
        if (isset($params['inputs']['admin_segment_ids'])) {
            ...
            $pv = PayeeVoucher::where('admin_segment_id', $params['inputs']['admin_segment_id'])
                ->selectRaw('admin_segment_id,hr_employees.id as employee_id,hr_employees.first_name as first_name,hr_employees.last_name as last_name,SUM(net_amount+total_tax) as amount')
                ->join('treasury_receipt_voucher_id_vouchers', 'treasury_receipt_voucher_id_vouchers.id', '=', 'treasury_payee_vouchers.payment_voucher_id')
                ->join('hr_employees', 'hr_employees.id', '=', 'treasury_payee_vouchers.employee_id')
